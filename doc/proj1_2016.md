...
1.  **(5 pts.) Set your development environment and install a custom Tizen kernel on your Z3 device**
    ...
    **Note: For virtual machine users, make sure that the Tizen USB device connects to guest OS and your virtualization software supports USB 3.0 port connection. (VirtualBox recently starts to support it since version 5.x)**
	...
	10. Copy the file to/from the device
		...
		In order to push some files to your device, you will have to the Smart Development Bridge ([sdb](https://developer.tizen.org/development/tools/common-tools/smart-development-bridge)) utility. sdb is a command line tool that makes you able to easily manage your device/emulator through your development system (desktop or laptop). You may usually use it to check the connection with the device/emulator (`sdb devices`), push or pull files (`sdb push` or `sdb pull`), and get a shell of the device/emulator (`sdb shell`). For details, you can see [this link](https://developer.tizen.org/development/tools/common-tools/smart-development-bridge)
